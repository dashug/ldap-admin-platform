    ...
    <el-card class="container-card" shadow="always">
      ...
      <el-dialog :title="dialogFormTitle" :visible.sync="updateLoading">
        ...
        >
          ...
          <p class="dialog-hint">填写第三方平台字段名，对应到本系统属性（如：钉钉的 name → 本系统 中文名字）。</p>

          <template v-if="relationType === '用户字段动态关联'">
      ...
      <el-dialog :title="dialogFormTitle" :visible.sync="dialogFormVisible">
        ...
        >
          <template v-if="relationType === '用户字段动态关联'">
            <el-form-item label="类型">
              <span class="dialog-type-tag">用户字段动态关联</span>
            </el-form-item>
            <el-form-item label="类型标志">
              <el-select
                v-model="userVal"
          ...
          <template v-else>
            <el-form-item label="类型">
              <span class="dialog-type-tag">分组字段动态关联</span>
            </el-form-item>
            <el-form-item label="类型标志">
              <el-select
                v-model="groupVal"
...
export default {
  ...
  methods: {
    ...
    search() {
      ...
      const keyword = (this.params.flag || '').trim().toLowerCase()
      if (keyword) {
        this.infoTableData = this.deal(this.infoTableData, (node) =>
          (node.Flag || '').toLowerCase().includes(keyword)
        )
      }
    },
    resetData() {
    ...
    getEmptyFormData() {
      return {
        username: '', nickname: '', givenName: '', mail: '', jobNumber: '', mobile: '',
        avatar: '', postalAddress: '', position: '', introduction: '', sourceUserId: '', sourceUnionId: '',
        groupName: '', remark: '', sourceDeptId: '', sourceDeptParentId: ''
      }
    },
    // 提交表单
    submitForm(e) {
      let flag, attributes
      if (this.relationType === '用户字段动态关联') {
        if (this.userVal === '') {
          Message({
            message: '请选择类型标志',
            type: 'warning'
