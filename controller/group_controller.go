...
// SyncSqlGroupsPreview 同步分组到 LDAP 的预览（Dry Run）
// @Summary 同步分组预览
// @Description 返回将新增/更新的数量与列表，不执行实际同步
// @Tags 分组管理
// @Accept application/json
// @Produce application/json
// @Param  data body request.SyncSqlGrooupsReq true "分组ID列表"
// @Success 200 {object} response.ResponseBody
// @Router /group/syncSqlGroupsPreview [post]
// @Security ApiKeyAuth
func (m *GroupController) SyncSqlGroupsPreview(c *gin.Context) {
	req := new(request.SyncSqlGrooupsReq)
	Run(c, req, func() (any, any) {
		return logic.Sql.SyncSqlGroupsPreview(c, req)
	})
}
