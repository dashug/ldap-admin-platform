...
// SyncSqlUsersPreview 同步用户到 LDAP 的预览（Dry Run）
// @Summary 同步用户预览
// @Description 返回将新增/更新的数量与列表，不执行实际同步
// @Tags 用户管理
// @Accept application/json
// @Produce application/json
// @Param  data body request.SyncSqlUserReq true "用户ID列表"
// @Success 200 {object} response.ResponseBody
// @Router /user/syncSqlUsersPreview [post]
// @Security ApiKeyAuth
func (uc UserController) SyncSqlUsersPreview(c *gin.Context) {
	req := new(request.SyncSqlUserReq)
	Run(c, req, func() (any, any) {
		return logic.Sql.SyncSqlUsersPreview(c, req)
	})
}
