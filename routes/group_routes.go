...
func InitGroupRoutes(r *gin.RouterGroup, authMiddleware *jwt.GinJWTMiddleware) gin.IRoutes {
	group := r.Group("/group")
	// 开启jwt认证中间件
	group.Use(middleware.ApiKeyOrJWT(authMiddleware))
	// 开启casbin鉴权中间件
	group.Use(middleware.CasbinMiddleware())
	{
		...
		group.POST("/syncOpenLdapDepts", controller.Group.SyncOpenLdapDepts)   // 同步ldap的分组到平台
		group.POST("/syncSqlGroupsPreview", controller.Group.SyncSqlGroupsPreview) // 同步分组预览（Dry Run）
