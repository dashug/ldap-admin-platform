...
func (l UserLogic) Update(c *gin.Context, req any) (data any, rspError any) {
	...
	if r.ExpireAt != nil && strings.TrimSpace(*r.ExpireAt) != "" {
		t, e := time.ParseInLocation("2006-01-02", strings.TrimSpace(*r.ExpireAt), time.Local)
		if e == nil {
			expireAt = &t
		}
	}
	...
	user := model.User{
		...
		ExpireAt:      expireAt,
		LastLoginAt:   oldData.LastLoginAt,
		Password:      oldData.Password,
		Status:        oldData.Status,
		SyncState:     oldData.SyncState,
		SourceUserId:  oldData.SourceUserId,
		SourceUnionId: oldData.SourceUnionId,
	}
