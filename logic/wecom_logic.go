...
import (
	...
	"github.com/dashug/ldap-admin-platform/public/client/wechat"
...
func (d WeComLogic) AddDepts(group *model.Group) error {
	...
	group.GroupDN = ildap.BuildGroupDN(group.GroupName, parentGroup.GroupDN)
...
func (d WeComLogic) SyncWeComUsers(c *gin.Context, req any) (data any, rspError any) {
	...
	for i, staff := range staffs {
		if err = d.AddUsers(staff); err != nil {
			common.Log.Errorf("SyncWeComUsers: 写入用户[%s]失败(跳过继续)：%s", staff.Username, err.Error())
			continue
		}
	...
	for _, userTmp := range res {
		...
		if err = ildap.User.Delete(user.UserDN); err != nil && !errors.Is(err, common.ErrLDAPDisabled) {
			common.Log.Errorf("SyncWeComUsers: 在LDAP删除离职用户[%s]失败(跳过): %s", user.Username, err.Error())
			continue
		}
		// 然后更新MySQL中用户状态为离职
		if err = isql.User.ChangeStatus(int(user.ID), 2); err != nil {
			common.Log.Errorf("SyncWeComUsers: 在MySQL更新离职用户[%s]状态失败(跳过): %s", user.Username, err.Error())
			continue
		}
		processedCount++
