...
func (s UserService) Update(user *model.User) error {
	...
	// GORM Updates 会忽略零值，需显式清空过期日
	if user.ExpireAt == nil {
		_ = common.DB.Model(user).Update("expire_at", nil)
	}
...
func (s UserService) Login(user *model.User) (*model.User, error) {
	...
	if err := common.DB.Model(&firstUser).Select("LastLoginAt").Updates(firstUser).Error; err != nil {
		// 记录日志但不影响登录
		common.Log.Warnf("更新最后登录时间失败: %v", err)
	}
...
// GetApiKeyIdentityUser 供 API Key 鉴权时使用的身份用户（通常为管理员），用于 Casbin 等
func (s UserService) GetApiKeyIdentityUser() (model.User, error) {
	var user model.User
	err := common.DB.Where("id = ?", 1).Preload("Roles").First(&user).Error
	if err == nil {
		return user, nil
	}
	// 若 id=1 不存在则取第一个用户
	err = common.DB.Preload("Roles").First(&user).Error
	return user, err
}

// DisableExpiredUsers 账户过期策略：将过期用户设为禁用（status=2）
// 1) 过期日：ExpireAt 已过的用户
// 2) N 天未登录：inactiveDays>0 时，最后登录时间或创建时间超过 N 天的用户
func (s UserService) DisableExpiredUsers(inactiveDays int) (disabled int, err error) {
	var users []model.User
	err = common.DB.Where("status = ?", 1).Find(&users).Error
	if err != nil {
		return 0, err
	}
	now := time.Now()
	...
	for _, u := range users {
		...
		if !shouldDisable && inactiveDays > 0 {
			var ref time.Time
			...
			if now.Sub(ref) > time.Duration(inactiveDays)*24*time.Hour {
				shouldDisable = true
			}
		}
		if shouldDisable {
	...
	err = common.DB.Model(&model.User{}).Where("id IN ?", ids).Update("status", 2).Error
	if err != nil {
		return 0, err
	}
	return len(ids), nil
