...
// appStartTime 应用启动时间（用于系统信息运行时长）
var appStartTime time.Time

// RecordAppStartTime 记录应用启动时间（应在 main 中尽早调用）
func RecordAppStartTime() {
	appStartTime = time.Now()
}

// GetAppUptime 返回应用运行时长
func GetAppUptime() time.Duration {
	return time.Since(appStartTime)
}

// GetDBStatus 返回数据库驱动名与连接状态（nil 表示正常）
func GetDBStatus() (driver string, err error) {
	driver = config.Conf.Database.Driver
	if DB == nil {
		return driver, fmt.Errorf("数据库未初始化")
	}
	sqlDB, err := DB.DB()
	if err != nil {
		return driver, err
	}
	return driver, sqlDB.Ping()
}
