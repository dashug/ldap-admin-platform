...
// ensureApiKeyPolicies 确保管理员角色拥有 API 密钥相关接口权限（每次启动执行，兼容旧库未包含该权限的情况）
func ensureApiKeyPolicies() {
	if CasbinEnforcer == nil {
		return
	}
	for _, p := range [][]string{
		{"admin", "/apiKey/list", "GET"},
		{"admin", "/apiKey/create", "POST"},
		{"admin", "/apiKey/delete", "POST"},
	} {
		_, _ = CasbinEnforcer.AddPolicy(p[0], p[1], p[2])
	}
}

// 初始化mysql数据
func InitData() {
	// 每次启动都确保管理员拥有 API 密钥接口权限
	ensureApiKeyPolicies()
	...
	apis := []model.Api{
		...
		{
			Method:   "POST",
			Path:     "/base/configImport",
			Category: "base",
			Remark:   "导入配置（目录与同步规则 JSON）",
			Creator:  "系统",
		},
		{
			Method:   "GET",
		...
		{
			Method:   "POST",
			Path:     "/user/syncSqlUsersPreview",
			Category: "user",
			Remark:   "同步用户到LDAP预览（Dry Run）",
			Creator:  "系统",
		},
		{
			Method:   "GET",
		...
		{
			Method:   "POST",
			Path:     "/group/syncSqlGroupsPreview",
			Category: "group",
			Remark:   "同步分组到LDAP预览（Dry Run）",
			Creator:  "系统",
		},
		{
			Method:   "GET",
